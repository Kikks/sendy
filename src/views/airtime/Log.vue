<template>
  <div>
    ...
    <div class="airtime-log">
      ...
      <div v-if="!isFetchingTransaction && !errorMessage && filteredTransactions && filteredTransactions.length < 1">
      ...
      <div v-if="!isFetchingTransaction && !errorMessage">
        ...
        <div class="search" v-if="shouldShowSearchBox">
          <div class="searchbox px-3 pb-3" v-on-clickaway="away">
            <div
              class="row pt-3"
              v-if="!isSearchLoading && !searchErrorMessage && searchTransactionResult.length < 1"
            >
            ...
            >
              <div class="row">
                ...
                <div class="col-6 text-right" :class="!transaction.status || transaction.status  === 'Failed' ? 'red' : 'green'">{{transaction.status ? transaction.status : 'N/A'}}</div>
              </div>
              <div class="row searchrow">
                <div class="col-6">{{ transaction.currencyCode }}{{transaction.amount}}</div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div v-for="(airtime) in filteredTransactions" :key="airtime.id" class="activityRow">
        <div class="row">
          <div class="col">
            <div class="row firstRow">
              <div class="col-6 blue">{{airtime.phoneNumber}}</div>
              <!-- <div class="col-6 text-right" :class="{red: !activity.add, green: activity.add}"> -->
              <div class="col-6 text-right" :class="!airtime.status || airtime.status  === 'Failed' ? 'red' : 'green'">{{airtime.status ? airtime.status : 'N/A'}}</div>
            </div>
